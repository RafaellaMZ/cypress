...
import login from '../../support/commandsContas'

describe('Should test at a fuctional level', () => {
    beforeEach(() => {
        cy.login('rafita@ita.edu', '1234')
        cy.resetApp()
    })
    ...
    it('Should not create an account with same name', () => {
        ...
        cy.get(loc.CONTAS.BTN_SALVAR).click()
        cy.get(loc.MESSAGE).should('contains', '400')
    })
    ...
    it('Should create a transaction', () => {
        ...
        cy.get(loc.MOVIMENTACAO.CONTA).click().select('Conta para movimentacoes')
        cy.get(loc.MOVIMENTACAO.STATUS).click()
        cy.get(loc.MOVIMENTACAO.BTN_SALVAR).click()
        cy.get(loc.MESSAGE).should('contain', 'sucesso')

        cy.get(loc.EXTRATO.LINHAS).should('have.length', 7)
        cy.xpath(loc.EXTRATO.FN_XP_BUSCA_ELEMENTO('Desc', '123')).should('exists')
    })
